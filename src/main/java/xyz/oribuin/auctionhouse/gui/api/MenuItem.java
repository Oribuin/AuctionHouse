...
public class MenuItem {
    ...
    private CommentedConfigurationSection config; // The config file for the menu
    ...
    private MenuItem(CommentedConfigurationSection config) {
        this.config = config;
     ...
     */
    public static MenuItem create(CommentedConfigurationSection config) {
        return new MenuItem(config);
    }

    /**
     * Place the item in the menu
     *
    ...
    public final void place(BaseGui gui) {
        ...
        if (this.slots.isEmpty()) {
            ...
            final boolean hasMultiSlots = this.config.get(this.itemPath + ".slots") != null;
            if (hasMultiSlots) {
                this.slots(AuctionUtils.parseList(this.config.getStringList(this.itemPath + ".slots")));
            }
        }

        final ItemStack item = this.customItem != null
                ? this.customItem
                : AuctionUtils.deserialize(this.config, this.player, this.itemPath, this.placeholders);
